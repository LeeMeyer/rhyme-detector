...
test("should detect long and short rhyming words", () => {
    ...
    expect(rhymes[0]).toEqual(expect.arrayContaining([ 'overnight', 'delight' ]));
    expect(rhymes[0].length).toBe(2);

    expect(rhymes[1]).toEqual(expect.arrayContaining([ 'intertwine', 'mine', 'shine', 'incline' ]));
    expect(rhymes[1].length).toBe(4);

    expect(rhymes[2]).toEqual(expect.arrayContaining(['emboss', 'loss', 'boss' ]));
    expect(rhymes[2].length).toBe(3);
})

test('man from snowy river rhyme detection should run faster than 100ms', async () => {
    let content = fs.readFileSync(path.resolve(__dirname, "the-man-from-snowy-river.txt")).toString();
